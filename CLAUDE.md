...
1. **Stage 1 — input enrichment.** Based on which fields are populated in `SearchRequest` (email / phone / fullName), runs the relevant subset of: `HunterIoLookup`, `ClearbitLookup`, `FullContactLookup`, `GravatarLookup`, `PhoneValidator`, `GitHubSearch`, `GitLabSearch`, `WebSearchProvider` (DDG Instant Answer + LinkedIn site-search via DDG HTML endpoint when `fullName` is present), `OrcidLookup`, `OpenAlexLookup` (last two fire on `fullName` alone — academic identity). Each provider's results are scanned for new handles/emails which feed `discoveredHandles` / `discoveredEmails` (`ConcurrentBag`). LinkedIn `/in/<handle>` URLs surfaced by the DDG site-search are extracted as handles and fan out into Stage 2.
...
Several providers exist in `Services/OsintProviders/` (e.g. `DomainWhoisLookup`, `PgpKeyserverLookup`, `WaybackMachineLookup`) that are registered in `Program.cs` but **not currently invoked** by `RealSearchService`. `ProfileVerifier` is invoked from inside `UsernameSearch.CheckPlatformAsync` — it runs a second pass on the HTML each platform already fetches, extracting display name / bio / location and a confidence score that flow into `OsintNode` children (labels `Name`, `📝 Bio`, `Location`, `Verification`).

**Token-match gate (priority-1 platforms).** `UsernameSearch.SearchAsync` accepts an optional `expectedNameTokens` collection. `RealSearchService` builds it from `request.FullName` (whitespace-split, diacritic-stripped, lowercased, length ≥ 2) and passes it on every Stage 2 call. For priority-1 platforms (GitHub, LinkedIn, X, Reddit, GitLab, Instagram, TikTok) `CheckPlatformAsync` drops a candidate node when its extracted display name + bio (after diacritic stripping) contains **none** of the tokens of length ≥ 3. The gate is bypassed when no tokens are supplied, when all tokens are too short to be discriminative, or when no profile text was extracted at all (auth-walled pages — Instagram/LinkedIn anonymous fetch — provide no evidence either way and are kept). Diacritic stripping (`StripDiacritics` in `UsernameSearch`, mirrored by `BuildNameTokens` in `RealSearchService`) means a query for "Kacper Gradoń" still matches a profile whose name renders as "Kacper Gradon".
